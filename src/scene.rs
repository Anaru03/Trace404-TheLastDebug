...
impl Geometry {
    pub fn intersect(&self, ray: &Ray) -> Option<f32> {
        match self {
            Geometry::Cube(cube) => cube.intersect(ray),
            Geometry::Sphere(sphere) => sphere.intersect(ray),
            Geometry::Cylinder(cylinder) => cylinder.intersect(ray),
        }
    }

    pub fn normal_at(&self, point: Vec3) -> Vec3 {
        match self {
            Geometry::Cube(cube) => cube.normal_at(point),
...
impl SceneObject {
    pub fn cube(cube: Cube, material: Material) -> Self {
        Self {
    ...
    pub fn sphere(sphere: Sphere, material: Material) -> Self {
        Self {
    ...
    pub fn cylinder(cylinder: Cylinder, material: Material) -> Self {
        Self {
            geometry: Geometry::Cylinder(cylinder),
            material,
        }
    }
...
fn add_mug(objects: &mut Vec<SceneObject>) {
    objects.push(SceneObject::cylinder(
        Cylinder::new(Vec3::new(-2.15, 0.0, -1.75), 0.32, -0.58, 0.05),
        Material::plastic(),
    ));
}

fn add_trace_lens(objects: &mut Vec<SceneObject>) {
    objects.push(SceneObject::sphere(
        Sphere::new(Vec3::new(2.10, -0.25, -1.70), 0.28),
        Material::screen(),
    ));
}
